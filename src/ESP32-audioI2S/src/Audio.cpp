...
extern void pwm_audio_set_param_(int rate, int bits, int ch);
#endif
...
///#define AUDIO_PIN_R  25
///#define AUDIO_PIN_L  32 
...
///pinMode(AUDIO_PIN_R, OUTPUT);
 ...
 /*   ledcSetup(0,16000,12);    // 625000 khz is as fast as we go w 7 bits
    ledcAttachPin(AUDIO_PIN_R, 0);
	///ledcDetachPin(AUDIO_PIN_R);
    ledcWrite(0,0);
	*/
	///digitalWrite(AUDIO_PIN_R, 0);
	
	///ledcDetachPin(AUDIO_PIN_R);
	///pinMode(AUDIO_PIN_R, ANALOG);
...
//*******************************************************************************
 /*int16_t         n_Sample[16];
 n_Sample[0]=Gain(m_outBuff[m_curSample * 2]);
 n_Sample[1]=Gain(m_outBuff[m_curSample * 2+2]);
 n_Sample[2]=Gain(m_outBuff[m_curSample * 2+4]);
 n_Sample[3]=Gain(m_outBuff[m_curSample * 2+6]);
 n_Sample[4]=Gain(m_outBuff[m_curSample * 2+8]);
 n_Sample[5]=Gain(m_outBuff[m_curSample * 2+10]);
 n_Sample[6]=Gain(m_outBuff[m_curSample * 2+12]);
 n_Sample[7]=Gain(m_outBuff[m_curSample * 2+14]);
//*******************************************************************************
/* uint8_t         o_Sample[16];
 o_Sample[0]=Gain(m_outBuff[m_curSample * 2])& 255;
 o_Sample[1]=Gain(m_outBuff[m_curSample * 2])>>8;
 o_Sample[2]=Gain(m_outBuff[m_curSample * 2+2])& 255;
 o_Sample[3]=Gain(m_outBuff[m_curSample * 2+2])>>8;
 o_Sample[4]=Gain(m_outBuff[m_curSample * 2+4])& 255;
 o_Sample[5]=Gain(m_outBuff[m_curSample * 2+4])>>8;
 o_Sample[6]=Gain(m_outBuff[m_curSample * 2+6])& 255;
 o_Sample[7]=Gain(m_outBuff[m_curSample * 2+6])>>8;*/
 uint16_t         n_Sample[512];
 /*n_Sample[0]=Gain_alt(m_outBuff[m_curSample * 2]);
 n_Sample[1]=Gain_alt(m_outBuff[m_curSample * 2+1]);
 n_Sample[2]=Gain_alt(m_outBuff[m_curSample * 2+2]);
 n_Sample[3]=Gain_alt(m_outBuff[m_curSample * 2+3]);
 n_Sample[4]=Gain_alt(m_outBuff[m_curSample * 2+4]);
 n_Sample[5]=Gain_alt(m_outBuff[m_curSample * 2+5]);
 n_Sample[6]=Gain_alt(m_outBuff[m_curSample * 2+6]);
 n_Sample[7]=Gain_alt(m_outBuff[m_curSample * 2+7]);
 n_Sample[8]=Gain_alt(m_outBuff[m_curSample * 2+8]);
 n_Sample[9]=Gain_alt(m_outBuff[m_curSample * 2+9]);
 n_Sample[10]=Gain_alt(m_outBuff[m_curSample * 2+10]);
 n_Sample[11]=Gain_alt(m_outBuff[m_curSample * 2+11]);
 n_Sample[12]=Gain_alt(m_outBuff[m_curSample * 2+12]);
 n_Sample[13]=Gain_alt(m_outBuff[m_curSample * 2+13]);
 n_Sample[14]=Gain_alt(m_outBuff[m_curSample * 2+14]);
 n_Sample[15]=Gain_alt(m_outBuff[m_curSample * 2+15]);

 n_Sample[16]=Gain_alt(m_outBuff[m_curSample * 2+16]);
 n_Sample[17]=Gain_alt(m_outBuff[m_curSample * 2+17]);
 n_Sample[18]=Gain_alt(m_outBuff[m_curSample * 2+18]);
 n_Sample[19]=Gain_alt(m_outBuff[m_curSample * 2+19]);
 n_Sample[20]=Gain_alt(m_outBuff[m_curSample * 2+20]);
 n_Sample[21]=Gain_alt(m_outBuff[m_curSample * 2+21]);
 n_Sample[22]=Gain_alt(m_outBuff[m_curSample * 2+22]);
 n_Sample[23]=Gain_alt(m_outBuff[m_curSample * 2+23]);
 n_Sample[24]=Gain_alt(m_outBuff[m_curSample * 2+24]);
 n_Sample[25]=Gain_alt(m_outBuff[m_curSample * 2+25]);
 n_Sample[26]=Gain_alt(m_outBuff[m_curSample * 2+26]);
 n_Sample[27]=Gain_alt(m_outBuff[m_curSample * 2+27]);
 n_Sample[28]=Gain_alt(m_outBuff[m_curSample * 2+28]);
 n_Sample[29]=Gain_alt(m_outBuff[m_curSample * 2+29]);
 n_Sample[30]=Gain_alt(m_outBuff[m_curSample * 2+30]);
 n_Sample[31]=Gain_alt(m_outBuff[m_curSample * 2+31]);
 */
...
///	int16_t val_word1=m_outBuff[m_curSample * 2+2*tmp]>>0;
///	int16_t val_word2=m_outBuff[m_curSample * 2+2*tmp+1]>0;

n_Sample[tmp]=Gain(m_outBuff[m_curSample * 2+0+tmp]);
///n_Sample[tmp*2+1]=Gain(m_outBuff[m_curSample * 2+1+tmp*2]);
	
/*	if (val_word1>val_word2) val_word=val_word1;
	if (val_word1<val_word2) val_word=val_word2;
*/	
	///val_word=val_word>>2;
	/*if (val_word<-8192) val_word=-8192;
	if (val_word>8192) val_word=8192;*/

	
/*	n_Sample[tmp*2]=Gain_alt(16384+val_word1);
	n_Sample[tmp*2+1]=Gain_alt(16384+val_word2);*/


}

///Serial.println(n_Sample[0]);

size_t cnt;
///pwm_audio_write_((uint8_t *)o_Sample, 2, &cnt, 5000 / portTICK_PERIOD_MS);
///delayMicroseconds(200);
pwm_audio_write_((uint8_t *)n_Sample, configtmp*2, &cnt, 1);
///delayMicroseconds(200);
...
if (Serial.available() > 0) {
	...
	uint32_t setupdata = Serial.parseInt(); //read int or parseFloat for ..float...

    // say what you got:
    Serial.print("I received: ");
    Serial.println(setupdata, DEC);

///pwm_audio_set_param_(setupdata, 16, 2);
  }
*/
...
////if ((_audio_r  > _audio_w-400) && incomingByte>20) incomingByte--;
////if ((_audio_r  < _audio_w-400) && incomingByte<100 ) incomingByte++;


/*Serial.print("W: ");
Serial.print(_audio_w);
Serial.print(" R: ");
Serial.println(_audio_r);
*/
  
///				       audio_write_16(n_Sample, 0, 8, 1);
////for (int32_t tmp=10;tmp>=0;tmp--) {

//if (tmp==0) audio_write_16(m_Sample, 1, 1, 1);
////if (tmp==0) audio_write_16(m_Sample, 0, 1, 1);


#define NOP __asm__ __volatile__ ("nop\n\t")  // delay ~4.1666666ns on a 240MHz ESP32


 
///if (tmp%4==0 || tmp%4==1 || tmp%4==2) {  
///if (tmp%4==0 || tmp%4==1 || tmp%4==2 || tmp%4==3) {  

///if (_audio_r > _audio_w) incomingByte--;
	
///delayMicroseconds(1);
//// for (uint32_t NOPtimmer=incomingByte;NOPtimmer>0;NOPtimmer--) NOP;
...
///					   delayMicroseconds((10000000/160000)); //6.25us
/*					   WS2812_LONG_DELAY();
					   WS2812_LONG_DELAY();
					   WS2812_LONG_DELAY();
*/					   
...
#else	
    if (sampRate>=44100 || sampRate<=48000) {
	    pwm_audio_set_param_(sampRate, 16, 2);
    } else {
        sampRate=44100;
	    pwm_audio_set_param_(sampRate, 16, 2);
